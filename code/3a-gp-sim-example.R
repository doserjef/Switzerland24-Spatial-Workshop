# 3a-gp-sim-example.R: this script fits a spatial occupancy model using
#                      a full Gaussian Process with a simulated data set
#                      for direct comparison with a spatial occupancy model
#                      that uses an NNGP.
rm(list = ls())
library(spOccupancy)
...
# for a different data set compared to the 3b-nngp-sim-example.R script.
set.seed(400)
J.x <- 40
J.y <- 40
...
w.means.gp <- apply(out$w.samples, 2, mean) + mean(out$beta.samples[, 1])
save(dat, w.means.gp, file = '~/Dropbox/talks/switzerland24/workshop/results/spPGOcc-GP-sims.rda')
